...
- `astrall_ros2/controller_node` is the ROS2 bridge for the robot base. It subscribes to `/cmd_vel`, clamps and maps velocities into `Twist2D{vx, vy, w}`, sends that command through `astrall_core::Backend`, publishes `/astrall/imu`, `/astrall/wheel_speeds`, `/astrall/status`, and `/diagnostics` from Backend telemetry/status, and stops on command timeout or backend errors. The package name is currently `astrall_base_driver`.
...
- `RealBackend` is SDK-backed when `ASTRALL_ENABLE_SDK=ON`. `backend: real` initializes the Astrall SDK and requests control by default.
